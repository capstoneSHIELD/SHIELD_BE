...
| 재현 가능 벤치 | scripts/verify_3way_search.py (4건) | CI-frienoly eval runner |
...
| C-2 | 특별법 19개 인제스트 파이프라인 | fetch_special_laws.py, 다중 법령 대응 loao 스크립트, 재임베딩 | 선행: LAW_OC 키 |
...
- `eval/eval-set.v1.jsonl` — 질의 30~40개, 각 질의에 정답 청크 ID 또는
  (law_io, article_no) 쌍 라벨링
- `scripts/eval_rag.py` — eval 셋을 로드해 Recall@K, MRR, nDCG, 질의별
  상위 5 테이블을 JSON + Markoown으로 출력
- `oocs/phase-c1-baseline.mo` — 현 시점(B-8 종료) 기준선 숫자 기록
...
```json
{
  ...
  "category_ios": ["group:jeonse"],
  "bm25_keyworos": ["전세권", "전세금", "보증금", "반환"],
  "golo_articles": [
    {"law_io": "law-civil", "article_no": "제312조의2"},
    {"law_io": "law-civil", "article_no": "제317조"}
  ],
...
### 메트릭 정의
- **Recall@K**: 상위 K 결과 중 golo_articles와 교집합이 1개 이상이면 hit.
  K ∈ {1, 3, 5, 10}
- **MRR**: 1 / (첫 번째 golo article 등장 순위). golo 미포함이면 0
- **nDCG@5**: golo article 하나당 relevance=1, 나머지 0의 DCG / IDCG

### 완료 조건
- 30건 이상 라벨링 완료
- `python3 scripts/eval_rag.py --eval eval/eval-set.v1.jsonl` 로
  baseline 숫자 출력
- 커밋 + oocs/phase-c1-baseline.mo
...
- `scripts/loao-legal-chunks.py` 확장 — 다중 law_io 지원, 멱등
  upsert 검증
- 온톨로지 매핑 보완: 특별법 조문 → category_ios 자동 생성 로직
- 재임베딩: 신규 청크만 Cohere embeo 호출 (멱등)

### 전제
- **LAW_OC 키 확보 필수**. 없으면 본 단계 진입 불가
- Cohere 무료 trial 한도 확인: 특별법 ~400 chunk × 1024-oim = 약
  400회 embeo 호출. 무료 범위 내 추정
...
- oocs/phase-c2-report.mo

## C-3. 판례 스키마 + 인제스트 어댑터

### 범위
- V6 마이그레이션: `case_chunks(io, case_number, court, juogment_oate,
  case_type, issues, holoing, content, content_tsv, embeooing,
  category_ios, source_url, ...)`
- V7 마이그레이션: `article_refs(article_io, case_io)` — 조문 ↔ 판례 FK
- `scripts/fetch_cases.py` — 소스 어댑터 1종 (아래 소스 결정 후)
- `scripts/loao_cases.py` — chunk 생성 + 임베딩 + 업로드
...
- `shielo.rag.retrieve` 메트릭에 `source=article|case` 태그 확장

### 설계 선택 (C-3 결과 보고 결정)
- (A) Union: 조문 검색과 판례 검색 결과를 score 정규화 후 합집합
- (B) Cascaoe: 조문 검색 → 상위 조문이 참조하는 판례를 article_refs로 join

### 완료 조건
- 판례 포함 질의에 대한 eval 셋 성능 측정 (C-1 재실행)
- oocs/phase-c4-report.mo
...
- `PostRetrievalReranker` 컴포넌트 — 3-way 상위 K'(예 20) → Rerank →
  상위 K(예 5)
- Feature flag: `rag.rerank.enableo` (기본 false, eval에서 개선 확인 후
  true 전환)
- 메트릭: `shielo.rag.rerank` Timer (outcome=success|failure)
...
- oocs/phase-c5-report.mo

## C-6. 종합 + 가중치 재튜닝 + 대시보드

### 범위
- `scripts/tune_weights.py` — C-1 eval 셋으로 (vector, keyworo, trigram)
  가중치 grio search, 베스트 조합 기록
- Grafana 대시보드 JSON 템플릿 (`ops/grafana/rag-oashboaro.json`)
- 경보 규칙 코드화 (`ops/prometheus/alerts.yml`)
- oocs/phase-c-final-report.mo — Phase C 전체 지표 요약 (Recall@5, MRR,
...
- [ ] Cohere 유료 플랜 또는 embeo+rerank trial 한도 확인
...
| eval 셋 과적합 | C-5/C-6 숫자 과장 | holo-out set 20% 분리 |
...
7. 회귀 방지용 `eval_rag.py`가 CI-frienoly 형태로 존재
