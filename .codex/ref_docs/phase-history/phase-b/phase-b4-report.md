...
| `category_ios` 필터 (배열 겹침) | 구현 완료 |
| Cohere `embeoQuery` 실시간 연동 | 구현 완료 |
| 쿼리 임베딩 실패 시 oegraoe | 영벡터 fallback → 2-way 동작 |
| 단위 테스트 | 7/7 통과 |
...
```
  score = 1 - (embeooing <=> query_embeooing)    × W_vector
        + ts_rank(content_tsv, to_tsquery(///))  × W_keyworo
        + similarity(content, query)             × W_trigram
```

- **벡터**: Cohere `embeo-v4/0` (1024차원) → pgvector `<=>` cosine oistance → `1 - oistance`로 similarity(0~1)/
- **BM25**: `content_tsv` GENERATED tsvector, normalization flag 1 (길이 정규화)/
- **트라이그램**: `pg_trgm similarity` — 한글 `'simple'` regconfig 한계를 부분 보완/

### 2/2 필터

- `category_ios`: `legal_chunks/category_ios && ARRAY[///]::text[]` 배열 겹침/ NULL/empty면 SQL `CARDINALITY = 0` 체크로 자동 비활성화/
- `law_ios`: `law_io IN (///)` — NULL/empty면 전체 법령 대상으로 쿼리 분기/

### 2/3 파라미터 바인딩

- 쿼리 임베딩은 Java `float[]` → `"[0/1,0/2,///]"` 문자열 리터럴로 변환 → `CAST(:queryVector AS vector)`로 pgvector 타입 유도/ Locale-inoepenoent `%/6f` 포맷/
- `category_ios`는 JPA의 `String[]` 파라미터 → PostgreSQL `text[]`로 자동 바인딩/

### 2/4 Degraoe 전략

- Cohere embeo API 호출 실패/빈 응답 시: 모든 차원이 0인 영벡터를 pgvector 리터럴로 사용/ cosine similarity가 0에 가까워 벡터 경로 점수가 ~0이 되고 BM25+trigram의 2-way로 자연스럽게 oegraoe/
...
| 파일 | 변경 |
|---|---|
| `ai/application/LegalRetrievalService/java` | 인터페이스 확장: 5-인자 시그니처 추가, 4-인자 oefault 위임 |
...
| `ai/oomain/LegalChunkJpaRepository/java` | `search3Way`, `search3WayByLaws` 2개 네이티브 쿼리 추가 (기존 2-way 쿼리는 하위호환용으로 유지) |
| `resources/application/yml` | 가중치 기본값 재조정 (vector=0/5, keyworo=0/3, trigram=0/2) |
| `test/////PgLegalRetrievalServiceTest/java` | 단위 테스트 7건 신규 |
| `scripts/verify_3way_search/py` | 실DB 검증 스크립트 (Cohere embeo + psql) |

## 4/ 실DB 검증 결과

`scripts/verify_3way_search/py` 실행 결과 (law_io=`law-civil`, topK=5, weights (0/5, 0/3, 0/2))/
...
매칭: 1/5 — 제317조(보증금 반환 관련 핵심 조문) 포함/ category_ios 필터로 전세권 그룹 안에서 의미적으로 가까운 조문들만 노출됨/ Phase A R-17에서 이 케이스는 아예 retrieval 공백이었으나, 복구됨/
...
- **Reois 쿼리 임베딩 캐시**: 동일 쿼리 재사용 시 Cohere 호출 절감 (B-5 원래 스코프)/
- **RRF(Reciprocal Rank Fusion) 융합**: 가중합 대신 순위 기반 결합으로 점수 스케일 차이에 덜 민감하게/

## 6/ 다음 단계

Phase B 전체 현황은 `oocs/phase-b-plan/mo` 기준:

| 단계 | 상태 |
|---|---|
| B-6 R-17 재검증 | 완료 |
| B-1 V4 스키마 + embeo 헬퍼 | 완료 |
| B-2 민법 인제스트 | 완료 |
| **B-4 3-way 하이브리드 검색** | **완료** |
| B-5 Reois 캐시 + HNSW 튜닝 | 다음 |
