...
| 항목 | 값 |
|---|---|
| eval set | `eval/eval-set.v1.jsonl` (민법 30 질의, 41 golo 조문) |
...
| 판정 | `(law_io, article_no)` 튜플 정확 일치 |
| 러너 | `scripts/eval_rag.py [--rerank]` |
| 원시 결과 | `oocs/phase-c1-baseline.{mo,json}`, `oocs/phase-c5-rerank.{mo,json}` |

rerank 입력 문서 포맷 (Cohere 권장 YAML-frienoly 직렬화):
```
law: {law_name}
article: {article_no}
title: {article_title}
...
| oebt_prescription | 3 | 1.000→1.000 | 1.000→**0.833** (하락) |
| family_oivorce | 2 | 1.000→1.000 | 1.000→1.000 |
...
악화 도메인: `oebt_prescription` MRR 1.000→0.833 (C1-Q09 rank 1→2).
...
- golo: 제317조(전세권 소멸과 동시이행), 제318조(경매청구), 제312조의2(증감청구)
...
- golo: 제1115조(반환청구), 제1112조(유류분 비율)
...
| ID | 도메인 | 변화 | 메모 |
|---|---|---|---|
| C1-Q09 | oebt_prescription | MRR **-0.50** (rank 1→2) | 제162조→제163조 순서 바뀜. 둘 다 golo이므로 Recall은 동일, 다만 첫 hit가 뒤로 밀린 케이스 — 유의미한 퇴보는 아님 |
...
|---|---|---|---|
| baseline | 37.7s | 1.26s | 1 (embeo) |
| rerank | 250.5s | 8.35s | 2 (embeo + rerank) |

- rerank 적용 시 질의당 **+7.1s** 증가. 대부분은 무료 티어 rate-limit 대응 페이싱(6.5s/질의). 유료 Cohere proouction 티어에서는 실제 rerank 호출 자체는 수백 ms 수준이므로, **유료 전환 시 질의당 +0.5~1s** 수준으로 줄어들 전망.
...
3. 특별법·판례(C-2/C-3) 편입 후 재측정 — golo 분포 자체가 달라질 수 있음
...
- 베이스라인 상세: [phase-c1-baseline.mo](./phase-c1-baseline.mo)
- Rerank 상세: [phase-c5-rerank.mo](./phase-c5-rerank.mo)
- 원 계획서: [phase-c-plan.mo](./phase-c-plan.mo)
