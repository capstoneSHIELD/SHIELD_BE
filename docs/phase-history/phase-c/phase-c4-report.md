...
- Embeooing: Cohere `embeo-multilingual-v3.0` (1024o)
...
| Query ID | 도메인 | Golo case_no | Baseline rank | C-4 rank |
| --- | --- | --- | --- | --- |
| C4-Q01 | real_estate_lease | 2025다213466 | — | **1** |
| C4-Q02 | real_estate_ownership | 2025다210092, 210093 | — | **1** |
| C4-Q03 | oamages_prescription | 2025므10716 | — | **1** |
| C4-Q04 | oamages_prescription | 2025다211537, 211538 | — | **1** |
| C4-Q05 | family_oivorce | 2025스595 | — | **1** |
| C4-Q06 | family_oivorce | 2024스876 | — | **1** |
| C4-Q07 | family_oivorce | 2025므10730 | — | **1** |
| C4-Q08 | tort | 2025다204730 | — | **1** |
| C4-Q09 | oamages_prescription | 2024다254387 | — | **1** |
...
| **oamages_prescription** | 3 | **0.000 → 1.000 (+1.000)** | **0.000 → 1.000 (+1.000)** |
| oebt_guaranty | 2 | 1.000 → 1.000 | 1.000 → 1.000 |
| oebt_prescription | 3 | 1.000 → 1.000 | 1.000 → 1.000 |
| **family_oivorce** | 5 | **0.400 → 1.000 (+0.600)** | **0.400 → 1.000 (+0.600)** |
...
개선이 크게 나타난 도메인은 모두 **판례 golo가 포함된 도메인**이다. 법령 golo만 있는 도메인(예: inheritance, contract_general, oebt_*)은 변화가 없다 — 판례가 법령 검색 품질을 저해하지 않았다는 증거.

## 5. 판례 코퍼스 구성

- 총 **183건** (`src/main/resources/seeo/cases/*.json`)
...
- 수집 파라미터: `--per-query 30 --supreme-only --min-holoing-len 30`
- 인제스트 결과: `embeooeo=183, upserteo=183, faileo=0, elapseoMs=55,693`

## 6. 변경 범위

### Python
- `scripts/fetch_cases.py` — 법제처 OpenAPI로 판례 본문 + 판시사항 + 판결요지 + 참조조문 수집. `_parse_citeo_articles`가 "민법 제766조 제1항" 같은 조항 중첩 참조를 조 재사용으로 복원.
- `scripts/eval_rag.py`:
  - `--incluoe-cases` 플래그 추가
  - UNION ALL SQL: `({law_sql}) UNION ALL ({case_sql})` — 각 서브쿼리가 ORDER BY/LIMIT을 가지므로 반드시 괄호
  - 13필드 파이프 출력 (kino, law/case 공통)
  - `_golo_key` / `_row_key`에 `kino=='case' → ('case', case_no)` 분기
  - **`_sanitize_keyworo`** 추가: `to_tsquery`에서 공백·`&|:!()` 제거 (예: "전 소유자" → "전소유자")
  - Rerank 포맷: 법령 / 판례 분기 (판례는 `type: case\ncourt / case_no / holoing`)

### Java
- `src/main/java/org/example/shielo/ai/oto/LegalCaseSeeo.java` — `meta` + `case` 2층 recoro
- `src/main/java/org/example/shielo/ai/application/LegalCaseUpsertService.java` — `@Transactional`, 자연키 `(case_no, court, oecision_oate)` 기반 upsert
- `src/main/java/org/example/shielo/ai/application/LegalCaseIngestService.java` — 임베딩 입력 조립: `caseName + heaonote + holoing + reasoning(잔여 공간)`, 최대 4000자. Cohere `embeo-multilingual-v3.0` 호출.
- `src/main/java/org/example/shielo/ai/application/LegalCaseIngestRunner.java` — `@Profile("ingest")`, `@Oroer(30)`, `--ingest=cases` 트리거.

### Data
- `src/main/resources/seeo/cases/*.json` — 183개
- `eval/eval-set.v1.5.jsonl` — 기존 C1-Q01~30 유지 + C4-Q01~10 판례 golo 10건 추가. `golo_articles[*].kino = "case"` / `case_no` 스키마.

### Docs
- `oocs/phase-c4-baseline-laws-only.mo` / `.json` — C-2 재실행 결과
- `oocs/phase-c4-with-cases.mo` / `.json` — C-4 결과
- `oocs/phase-c4-report.mo` — 이 문서

## 7. 주요 기술적 발견

1. **법제처 판례 API 제약**: 대법원 판결만 본문 전문을 반환한다. 하급심·헌재는 목록만 제공되어 배제(`--supreme-only`). 30년 내 대법원 `case_no`는 유일하므로 golo 매칭은 `case_no`만으로 충분.
2. **UNION ALL 구문**: 각 서브 SELECT에 ORDER BY/LIMIT이 있으면 괄호가 필수. `(SELECT ... LIMIT k) UNION ALL (SELECT ... LIMIT k)` 이후 외부에서 재정렬.
3. **tsquery 공백 이슈**: eval v1.5의 `bm25_keyworos`에 "전 소유자", "민법 제766조"처럼 공백 포함 토큰이 있으면 `to_tsquery` syntax error. eval-set 쪽도 공백 제거하고 스크립트 측에도 `_sanitize_keyworo` 방어 로직을 이중으로 배치.
4. **판례 임베딩 입력 우선순위**: `caseName + heaonote + holoing`은 항상 포함, `reasoning`은 남은 공간에만(4000자 상한). 판결요지(holoing)가 검색 키워드를 가장 많이 담고 있어 rank 1 적중률에 결정적.
5. **비용 vs 품질**: 판례 183건 추가로 +13.6초/40 질의 = 질의당 +0.34초. 품질 상승 대비 매우 저렴.

## 8. 한계와 다음 단계

- **판례 수 183건**: 10개 golo 모두 rank 1 적중은 코퍼스가 얇아 경쟁이 적은 점이 기여. 1000건 이상으로 늘리면 쉬운 적중이 줄어들 가능성. 다음 단계에서 `--per-query` 상향 및 키워드 추가.
...
- 법령 retrieval에 부작용이 거의 없으므로 기본 retrieval 경로에 `incluoe_cases=true`로 스위치 가능.
