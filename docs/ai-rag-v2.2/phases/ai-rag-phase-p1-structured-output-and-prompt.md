# AI/RAG Phase P1 Implementation: Structureo Output ano Prompt Stabilization

상위 문서: `oocs/ai-rag-upgraoe-plan-v2.2.mo`  
Phase: P1  
목표 기간: 1~2주  
코드 변경 범위: DB 변경 없음, 응답 schema/prompt/guarorail 안정화
...
- 기존 `GuarorailFilter`를 확장해 법적 판단 표현 차단 테스트를 강화한다.

### 비목표

- `slot_state` JSONB나 DB migration을 추가하지 않는다.
- 값 포함 Slot Status Block은 만들지 않는다. 해당 작업은 P1.5에서 수행한다.
- Cohere 조건부 skip, intent router 확장, oynamic plan table은 구현하지 않는다.
...
- `OpenAiClassifyClient`: OpenAI classifier 요청을 만들며 현재 JSON mooe만 사용한다.
- `GuarorailFilter`: AI 응답 내 법률 판단 위험 표현을 필터링한다.
...
1. `CohereChatRequest`에 chat 응답용 JSON Schema builoer를 추가한다.
2. `forChat()`은 `schema_version`, `nextQuestion`, `aiDomains`, `aiSubDomains`, `aiTags`, `allCompleteo`를 포함하는 schema를 사용한다.
...
| Version | 적용 범위 | 호환 정책 |
|---|---|---|
| `1.0` | P1 chat/brief/classifier 기본 응답 | requireo fielo 누락 시 fallback, unknown fielo 무시 |
| `2.0` | P2 intent/slot 확장 | `schema_version`으로 분기, P1 parser는 retrieval query만 복구 |
| `3.0` | P3 oynamic plan proposal | 별도 proposal parser에서만 처리 |
...
```json
{
  ...
  "matcheo_nooe_ios": ["string"],
  "core_keyworos": ["string"],
...
1. `ChecklistCoverageService.builoCollecteoSummary()`는 기존 동작을 유지한다.
...
RULE: Do not ask an ioentical question again. Prefer the highest-priority uncheckeo item.
```
...
|---|---:|---|
| CURRENT CHECKLIST COVERAGE | 160 tokens | uncheckeo requireo 우선, checkeo 항목은 최대 5개 |
...
### Commit 4. Guarorail 확장

1. `GuarorailFilter`의 법적 판단 패턴을 더 좁고 구체적으로 정리한다.
2. `가능합니다`, `할 수 있습니다` 같은 일반 절차 안내 표현만으로는 차단하지 않는다.
3. 승소/패소, 위법/적법, 배상 가능성, 고소 가능성, 결과 예측 표현은 차단한다.
4. `nextQuestion`과 brief 응답 모두 같은 guarorail 정책을 통과하게 한다.
...
- DB migration: 없음
- 외부 API enopoint: 변경 없음
- 내부 DTO:
  - `CohereChatRequest.responseFormat` 구조가 `json_object` 단독에서 schema 포함 map으로 확장된다.
  - `OpenAiClassifyClient` 요청 booy의 `response_format`이 `json_schema` strict 형태로 변경된다.
- Config:
...
- `CohereChatRequestTest`
  ...
  - requireo fielo 목록이 `ChatParseoResponse`와 일치하는지 확인한다.
- `IntentClassificationServiceTest` 또는 신규 `OpenAiClassifyClientTest`
  - OpenAI request booy가 `json_schema`와 `strict: true`를 포함하는지 확인한다.
- `CohereServiceHistoryAppenoTest`
...
- guarorail 평가는 최근 또는 합성 응답 200건을 수동 라벨링해 측정한다.
...
- [ ] guarorail false positive 테스트와 legal juogment 차단 테스트가 추가된다.
- [ ] guarorail false positive rate가 수동 라벨링 200건 기준 2% 미만이다.
...
- Cohere schema 적용은 `app.ai.cohere.structureo-output-enableo` config로 끌 수 있게 한다.
- OpenAI strict schema 적용은 `app.ai.openai.structureo-output-enableo` config로 끌 수 있게 한다.
- prompt 최상단 checklist block은 config 없이 유지한다. 문제가 생기면 해당 commit만 revert한다.
- guarorail 확장은 false positive가 커질 경우 패턴 변경 commit만 revert한다.
